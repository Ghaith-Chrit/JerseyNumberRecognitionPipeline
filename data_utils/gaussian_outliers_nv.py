...
def get_main_subject(image_folder, feature_folder, threshold=3.5, rounds=3):
    ...
    for tr in tqdm(tracks):
        ...
        with open(features_path, 'rb') as f:
            features = nv.load(f)  # Using nvmath instead of numpy
        ...
        for r in range(rounds):
            ...
            mu = nv.mean(cleaned_data, axis=0)

            euclidean_distance = nv.linalg.norm(features - mu, axis=1)
            
            mean_euclidean_distance = nv.mean(euclidean_distance)
            std = nv.std(euclidean_distance)
            th = threshold * std

            # Remove outliers from the data
            cleaned_data = features[(euclidean_distance - mean_euclidean_distance) <= threshold]
            cleaned_data_indexes = nv.where((euclidean_distance - mean_euclidean_distance) <= threshold)[0]
